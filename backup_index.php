					      ...
					      <tr>
					        ...
					        <table border="0" cellpadding="0" cellspacing="0" width="600" bgcolor="#fff">
					          <tbody>
					              ...
					              <tr>
					                <td style="padding: 45px 0px 0px 45px; font-size:18px;" >Dear '.$first_name.' '.$last_name.',</td>
					              </tr>
					              <tr>
					      ...
					      <tr>
					        ...
					        <table border="0" cellpadding="0" cellspacing="0" width="600" bgcolor="#fff">
					          <tbody>
					              ...
					              <tr>
					                <td style="padding: 45px 0px 30px 45px; font-size:18px;" >Dear '.$userdetail['first_name'].' '.$userdetail['last_name'].',</td>
					              </tr>
					              <tr>
					      ...
					      <tr>
					        ...
					        <table border="0" cellpadding="0" cellspacing="0" width="600" bgcolor="#fff">
					          <tbody>
					              ...
					              <tr>
					                <td style="padding: 0px 0px 0px 45px; font-size:18px;"></br>'.$userdetail['first_name'].' '.$userdetail['last_name'].' sent an invite to join UTC U-Nite App.</p>
...
/*
		...
		return array("err-code"=>"0","message"=>"Updated successfully.","quote"=>$quotes,"profile_image"=>$path,"thumnail"=>$thumb,"phone_no"=>$contact);
	}
...
function my_groups($data)
{
	...
	if ($user) 
	{
...

		{
			 ...
			 $detail['group_name'] = base64_decode($detail['group_name']);
...
function convert_group()
{
	...
	while ($detail = mysqli_fetch_assoc($query)) 

		{
			...
			$group_detail  = 	base64_encode($detail['group_detail']);

			$group_id = $detail['group_id'];
			mysqli_query($con,"update uwi_groups set group_original_name='$group_name' where group_id='$group_id'"); 
		}
...

{
	...
	if ($user) 
	{
		...
		$query = mysqli_query($con, "SELECT * FROM `uwi_groups` where owner_type IN ('Admin','User') and case when owner_type='Admin' then group_type!='Private' else 1 end order by group_id desc LIMIT $post_value,10 ");
...

{
	...
	if ($user) 
	{
		...
		$query = $detail = mysqli_fetch_assoc(mysqli_query($con, "select *,(select count(*) as tlike from uwi_likes where uwi_likes.ref_id=uwi_groups.group_id and uwi_likes.uid='$user_id' and uwi_likes.source='group' ) as you_like ,(select moods from uwi_likes where uwi_likes.ref_id=uwi_groups.group_id and uwi_likes.uid='$user_id' and uwi_likes.source='group' ) as your_like,(select count(*) as tlike from uwi_likes where uwi_likes.ref_id=uwi_groups.group_id and  uwi_likes.source='group' ) as total_likes from uwi_groups where group_id='$group_id' "));


		 $detail['group_name'] = base64_decode($detail['group_name']);

		 $detail['group_detail'] = base64_decode($detail['group_detail']);
...

{
	...
	if ($user) 
	{
		...
		$query = mysqli_query($con, "select * from uwi_groups where (LOWER(group_original_name) LIKE LOWER('%$search_text%') or LOWER(group_tags) LIKE LOWER('%$search_text%')) and owner_type IN ('Admin','User') and case when owner_type='Admin' then group_type!='Private' else 1 end order by group_id desc LIMIT $post_value,10 ");
...
function push_notification($data)
{
	...
	if ($user) 
	{
		...
		$message = $userProfile['first_name'].' '.$userProfile['last_name'].' commented on '.$source .".";
		 ...
		 if($source=='group')
		 {
		 	...
		 	$message = $userProfile['first_name'].' '.$userProfile['last_name']." has commented in the {$group_info['group_name']} ".$source .".";
			...
			if($user_id!=$uid)
			{
				if($source=='group')
				{
					if($device_list['mute_group']=='0' )
					{
						...
						if($device_type=='android')
						{
						  echo send_notification_android($device_token,$message,$source,$ref_id);
						}
					}
				}
				else
...

{
...

	{
...

		{
			...
			$tok = mysqli_fetch_assoc(mysqli_query($con,"select uwi_groups.group_name,uwi_groups.owner_id,uwi_users.device_token,uwi_users.device_type,uwi_users.badge,uwi_users.phone_no from uwi_groups join uwi_users on uwi_users.uid=uwi_groups.owner_id where group_id='$group_id' and owner_type='User'"));
		//echo "select uwi_groups.group_name,uwi_groups.uid,uwi_users.device_token,uwi_users.badge from uwi_groups join uwi_users on uwi_users.uid=uwi_groups.uid where group_id='$group_id' and owner_type='User'";
			...
			$message = $userProfile['first_name'].' '.$userProfile['last_name'].' has sent a request to join the '.$group_name.' group';
...
function request_status($data)
{
	...
	if ($user) 
	{
		...
		if($member_status=='Accept')
		{
			...
			$tok = mysqli_fetch_assoc(mysqli_query($con,"select uwi_groups.group_name,uwi_groups.owner_id from uwi_groups where group_id='$group_id' and owner_type='User'"));
			...
			$message = $userProfile['first_name'].' '.$userProfile['last_name'].' has accepted your request to join the '.$tok['group_name'].' group';
...
//date_default_timezone_set("Asia/Kolkata");
...

	{
		...
		$content = mysqli_real_escape_string($con,base64_encode($data->content));
...
//date_default_timezone_set("Asia/Kolkata");
...

	{
		...
		{
			$list['content'] = base64_decode($list['content']);
...

{
...

	{
		...
		$group_name=mysqli_real_escape_string($con,base64_encode($data->group_name));

		$group_original_name=mysqli_real_escape_string($con,$data->group_name);
		...
		$group_detail=mysqli_real_escape_string($con,base64_encode($data->group_detail));
		...
		mysqli_query($con,"insert into `uwi_groups` (`group_name`,`group_original_name`, `group_detail`,`group_create_date`,`group_tags`,`owner_id`,`group_type`,`owner_type`) values('$group_name','$group_original_name','$group_detail',NOW(),'$group_tags','$user_id','$group_type','User')" );  
...

{
...

	{
		...
		mysqli_query($con,"update uwi_groups set group_name='$group_name',group_original_name='$group_original_name', group_detail='$group_detail',group_tags='$group_tags',group_type='$group_type' where group_id='$group_id'");
